...
export default function WaiterPhotoDishCard({
  ...
  const soldOut = Boolean(item.out_of_stock);
  ...
  return (
    <div className="relative">
      <button
        ...
        className={`w-full overflow-hidden rounded-lg text-left shadow-md min-h-[96px] select-none ${
          canOrder || soldOut ? 'bg-zinc-900' : 'bg-gray-400 opacity-50 cursor-not-allowed'
        }`}
        {...longPress}
      >
        <div className={`relative aspect-[4/3] w-full bg-zinc-800 ${soldOut ? 'opacity-60' : ''}`}>
          {cachedSrc || photoSrc ? (
            <img
          ...
          <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent px-1.5 py-1.5">
            ...
            <p className="mt-0.5 text-[10px] font-bold text-white">{formatPrice(item.price)}</p>
          </div>
          {soldOut ? (
            <div className="absolute inset-0 flex items-center justify-center bg-black/45">
              <span className="rounded bg-white px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-gray-900">
                Sold out
              </span>
            </div>
          ) : null}
      ...
      {isPopular ? (
        <span className="absolute bottom-1 right-1 rounded bg-amber-400 px-1 py-0.5 text-[8px] font-bold text-amber-950 pointer-events-none">
          Popular
        </span>
      ) : null}

      {inCartQty > 0 ? (
        <span className="absolute bottom-7 right-1 min-w-[18px] rounded-full bg-green-600 px-1 text-center text-[10px] font-bold text-white pointer-events-none">
          {inCartQty}
        </span>
      ) : null}
...
export function WaiterTextDishCard({
  item,
  inCartQty,
  isPopular,
  isFavorite,
  ...
  return (
    <div className="relative">
      <button
        ...
        onClick={onAdd}
        onContextMenu={(event) => event.preventDefault()}
        className={`w-full p-1.5 sm:p-2 rounded-lg text-center font-medium min-h-[64px] sm:min-h-[72px] flex flex-col justify-center transition-all duration-300 shadow-md hover:shadow-lg select-none ${
          soldOut
            ? 'bg-gradient-to-br from-gray-500 to-gray-700 cursor-pointer'
            : canOrder
      ...
      >
        <div
          className="font-semibold text-[10px] sm:text-[11px] leading-tight px-0.5 overflow-hidden text-white"
          style={{
            display: '-webkit-box',
            WebkitLineClamp: 2,
        ...
        <div className="text-[9px] sm:text-[10px] opacity-90 mt-0.5 bg-white/20 rounded px-0.5 py-0.5 text-white">
